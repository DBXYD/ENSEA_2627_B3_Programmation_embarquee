...
Ce TP a pour objectif d'introduire aux étudiants mes bases de la robotique embarquée en utilisant une Rasberry Pi comme plateforme de contrôle. A travers les différentes étapes, configuration du système, lecture des capteurs, traitement des données et pilotage d'un moteur, les étudiants découvrent concrètement comment un robot perçoit, interprète et réagit à son environnement.
...
A l'issue de ce TP, les étudiant sont capables d'interfacer du matériel réel, d'exploiter des données capteurs et de programmer des actions correspondant à un comportement robotique simple. 

## Introduction
Une RPI (Rasberry Pi) est un petit ordinateur conçu pour l’apprentissage de la programmation et des bases informatiques. Elle dispose d’un processeur, d’une mémoire vive (RAM = Random Access Memory), d’un espace de stockage (via une carte micro SD), ainsi que de ports USB, HDMI et d’une connectivité réseau sans fil (wifi).  
...
Pour pouvoir établir une connexion avec la RPI, on utilise le connexion SSH (Secure Shell), c’est un protocole de communication qui permet de se connecter à un autre ordinateur via un réseau IP. Il nous fournit une interface en ligne de commande à travers laquelle on peut exécuter des instructions, transférer des fichiers ou administrer un système, comme si on travaillait dessus directement. Dons nous l’utiliserons pour pouvoir se connecter à notre Raspberry via notre ordinateur. 
